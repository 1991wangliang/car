...
public class CarParkingTest {
    ...
    private int gridWidth = width / 32;
    private int gridHeight = height / 32;
    ...
    public void test(String pitcute) {
        NativeImageLoader imageLoader = new NativeImageLoader(height, width, nChannels);
        ImagePreProcessingScaler scaler = new ImagePreProcessingScaler(0, 1);
        org.deeplearning4j.nn.layers.objdetect.Yolo2OutputLayer yout = (org.deeplearning4j.nn.layers.objdetect.Yolo2OutputLayer) model.getOutputLayer(0);
        List<String> labels = Arrays.asList("car", "parking");
        INDArray features = null;
        try {
            features = imageLoader.asImageMatrix(new File(carConfig.getPath() + "/" + pitcute)).getImage();
